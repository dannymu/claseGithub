...
adapter_name_to_activate = loaded_adapter if loaded_adapter is not None else args.adapter_load_as

model.set_active_adapters(adapter_name_to_activate)

print("Adapter solicitado:", args.adapter_name)
print("Adapter cargado como:", adapter_name_to_activate)
print("Adapters disponibles:", list(model.adapters_config.adapters.keys()))
print("Adapter activo:", model.active_adapters)

if model.active_adapters is None:
    raise RuntimeError(
        "No hay adapter activo. Deteniendo ejecución para evitar embeddings sin adapter de clasificación."
    )
