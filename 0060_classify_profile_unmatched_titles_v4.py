# ============================================================
# 0060_classify_profile_unmatched_titles_v4.py
#
...
#
# Corrección v4:
# - Fix indentación: output = model(**inputs) dentro de AdapterSetup
# - Se usa AdapterSetup en cada forward pass (no set_active_adapters global)
#   para evitar que el test de verificación restablezca estado a None.
# ============================================================
...
def format_title_for_specter2(title, sep_token):
    # SPECTER2 fue entrenado con 'title [SEP] abstract'.
    # Para título-solo: title [SEP] "" (abstract vacío).
...
# Verificación: forward de prueba con AdapterSetup
...
with torch.no_grad():
    for start in tqdm(range(0, n_docs, args.batch_size)):
        ...
        # ---- FIX v4: output DENTRO del with AdapterSetup ----
        with AdapterSetup(adapter_name):
            output = model(**inputs)
        # -----------------------------------------------------
...
for start in tqdm(range(0, n_docs, 2000)):
    ...
    for i in range(n_batch):
        ...
        if top1_score < args.min_score:
            status, conf, strength = "low_similarity_review",    "baja_similitud",            "exploratory_title_only_low_similarity"
        elif margin < args.min_margin:
            status, conf, strength = "ambiguous_review",         "ambiguo",                   "exploratory_title_only_ambiguous"
        elif top1_score >= 0.60 and margin >= 0.05:
            status, conf, strength = "preliminary_title_only",   "alta_confianza_title_only", "exploratory_title_only_high_signal"
        elif top1_score >= 0.45 and margin >= 0.02:
            status, conf, strength = "preliminary_title_only",   "confianza_media_title_only","exploratory_title_only_medium_signal"
        else:
            status, conf, strength = "preliminary_title_only",   "revisar_title_only",        "exploratory_title_only_review"
        ...
        pub_row["review_reason"]               = "title_only_no_corpus_match; exploratory_zero_shot_suggestion"
